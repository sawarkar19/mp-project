...
function currency_info()
{
	...
	if (Cache::has($url)) {
		...
		$r=cache()->remember('currency_info', 300, function () {
			$currency=\App\Models\Option::where('key','currency_info')->first();
...
if(!function_exists("businessWizardSetup")){
	function businessWizardSetup($user_id)
	{
		$businessDetail = BusinessDetail::whereUserId($user_id)->first();
		return $businessDetail['is_wizard_setup'];
	}
}