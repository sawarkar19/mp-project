...
class PlanController extends Controller
{
    ...
    public function planHistory(Request $request ,$id)
    {
        ...
        return view('support.transaction',compact('plans','notification_list','planData'));
    }
    ...
    public function viewTransactionHistory($id)
    {
        ...
        if(!empty($transaction) && $transaction->id == $id){
            ...
            return view('support.viewTransactionHistory',compact('transaction','notification_list','planData','plan_period', 'count_free_paid_employee_ids', 'employee_price', 'empPlanData', 'rechangeInfo'));
        }else{
            abort(404);
        }
    }
    ...
    public function viewTransactionInvoice($id)
    {
        ...
        if(!empty($transaction) ){
            ...
            $pdf = \PDF::loadView('support.download-history',compact('transaction','plan_period','business','user', 'count_free_paid_employee_ids', 'employee_price', 'empPlanData', 'rechangeInfo'))->setOptions(['defaultFont' => 'sans-serif', 'enable_remote' => true])->setPaper($customPaper, 'portrait');
