...
class InstantTaskController extends Controller
{
    ...
    public function sharedInstantTemplate(Request $request,$offer_uuid){
        ...
        if($offer != null){
            ...
            if(empty($request->all())){
                ...
                $instantTasks = InstantTask::with('activeTask', 'task')->has('activeTask')->where('user_id',$offer->user_id)->whereIn('id', $showInstantTaskIds)->whereNull('deleted_at')->whereNotIn('id', $anotherCompletedTasks)->whereNotIn('id', $socialPostDisconnectedInstantTaskIds['instantTaskIds'] ?? [])->orderBy('task_id','ASC')->groupBy('task_id')->get();
    ...
    public function facebookPostComment(Request $request)
    {
        ...
        $fb_task_ids = [2, 3, 15];
        $instantTaskIds = InstantTask::with('task')->whereIn('task_id', $fb_task_ids)->where('user_id', $offer->user_id)->whereNull('deleted_at')->groupBy('task_id')->pluck('id')->toArray();
        ...
        $instantTasks = InstantTask::with('task')->whereNotIn('id', $completeTasks)->whereIn('task_id', $fb_task_ids)->where('user_id', $offer->user_id)->whereNull('deleted_at')->groupBy('task_id')->orderBy('created_at', 'DESC')->get();
    ...
    public function instagramPostLike(Request $request)
    {
        ...
        $task_ids = [5, 16];
        $instantTaskIds = InstantTask::with('task')->whereIn('task_id', $task_ids)->where('user_id', $offer->user_id)->whereNull('deleted_at')->groupBy('task_id')->pluck('id')->toArray();

        // Check Completed Tasks
        $completeTasks = CompleteTask::whereIn('instant_task_id', $instantTaskIds)->where('user_id', $offer->user_id)->where('customer_id', $customer_id ?? 0)->pluck('instant_task_id')->toArray();
        
        $instantTasks = InstantTask::with('task')->whereNotIn('id', $completeTasks)->whereIn('task_id', $task_ids)->where('user_id', $offer->user_id)->whereNull('deleted_at')->groupBy('task_id')->orderBy('created_at', 'DESC')->get();
    ...
    public function verifyYoutubeComment(Request $request){
        ...
        $instantTaskIds = InstantTask::with('task')->whereIn('task_id', $youtube_task_ids)->where('user_id', $offer->user_id)->whereNull('deleted_at')->groupBy('task_id')->pluck('id')->toArray();

        // Check Completed Tasks
        $completeTasks = CompleteTask::whereIn('instant_task_id', $instantTaskIds)->where('user_id', $offer->user_id)->where('customer_id', $customer_id ?? 0)->pluck('instant_task_id')->toArray();
        
        $instantTasks = InstantTask::with('task')->whereNotIn('id', $completeTasks)->whereIn('task_id', $youtube_task_ids)->where('user_id', $offer->user_id)->whereNull('deleted_at')->groupBy('task_id')->orderBy('created_at', 'DESC')->get();
