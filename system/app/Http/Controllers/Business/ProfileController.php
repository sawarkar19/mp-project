...
class ProfileController extends Controller
{
    ...
    {   
        ...
        return view('business.profile-setting', compact('basic','notification_list','planData', 'userData', 'logo_url'));
    }
    ...
    public function emailUpdate(){
        $check = User::where('email', $_POST['email_id'])->orderBy('id', 'desc')->first();

        if ($check != null) {
            if ($check->id == Auth::id()) {
                echo json_encode(array("type"=>"error", "message"=> "This email is already used to your account."));
            } else {
                echo json_encode(array("type"=>"error", "message"=>"Opps the email id already exists...!!"));
            }
        }else {
            $checkCode = IpUser::where('email', $_POST['email_id'])->where('date', Carbon::today())->count();
            ...
            if ($checkCode <= 2) {
                ...
                /* Send Mail */
                \App\Http\Controllers\CommonMailController::OtpVerificationMail($data);

                $clientIP = request()->ip();

                $otp_save = new IpUser;
                $otp_save->user_ip = $clientIP;
                $otp_save->email = $email;
                $otp_save->date = Carbon::today();
                $otp_save->is_sent = 1;
                $otp_save->save();

                echo json_encode(array("type" =>"success", "OTP sent on your email."));
            }
            else{
                echo json_encode(array("type"=>"error","message"=>"OTP usage limit exceeded for this email. Try another email."));
            }
        }
    }
    ...
    public function verifyUpdateEmail(){      
        ...
        if ($isExist) {
            ...
            User::where('id',$user_id)->update(['email'=>$results[0]->email]);

            echo json_encode(array("type"=>"success", "session_email"=>$results[0]->email, "message"=>"Email id successfully updated!"));        
