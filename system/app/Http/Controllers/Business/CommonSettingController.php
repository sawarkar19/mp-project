...
class CommonSettingController extends Controller
{
    ...
    public function refreshSettingsStatus(Request $request){
        $planData = array();
        if($request->_token){
            /* Business Details */
            $business_detail = BusinessDetail::where('user_id', Auth::id())->first();
            ...
            $getYoutubeTasks = InstantTask::where('user_id', Auth::id())->whereIn('task_id', [10,11,12])->whereNull('deleted_at')->count();

            $instantChallengeOfferReward = OfferReward::where('user_id', Auth::id())->where('channel_id', 2)->first();
            $sharedChallengeOfferReward = OfferReward::where('user_id', Auth::id())->where('channel_id', 3)->first();
            ...
            // 2. Setting
            $planData['statusbar']['setting']['business_details'] = (isset($business_detail->business_name) && $business_detail->business_name!=NULL) ? 1 : 0;
            
            $planData['statusbar']['setting']['business_contact'] = (isset($business_detail->call_number) && $business_detail->call_number!=NULL) && (isset($business_detail->address_line_1) && $business_detail->address_line_1!=NULL) && (isset($business_detail->pincode) && $business_detail->pincode!=NULL) && (isset($business_detail->city) && $business_detail->city!=NULL) && (isset($business_detail->state) && $business_detail->state!=NULL) ? 1 : 0;
            
            $planData['statusbar']['setting']['business_address'] = (isset($business_detail->billing_address_line_1) && $business_detail->billing_address_line_1!=NULL) && (isset($business_detail->billing_pincode) && $business_detail->billing_pincode!=NULL) && (isset($business_detail->billing_city) && $business_detail->billing_city!=NULL) && (isset($business_detail->billing_state) && $business_detail->billing_state!=NULL) ? 1 : 0;

            $planData['statusbar']['setting']['social_link'] = (isset($business_detail)) && ($business_detail->facebook_link!=NULL || (isset($business_detail->instagram_link) || $business_detail->twitter_link!=NULL) || $business_detail->linkedin_link!=NULL || $business_detail->youtube_link!=NULL) ? 1 : 0;
            ...
            $vcard_setting = 0;
            if($business_detail->vcard_type =="vcard"){
                $vcard_setting = ($business_detail->business_card_id != NULL) ? 1 : 0;
            }
            else{
                $vcard_setting = ($business_detail->webpage_url != NULL) ? 1 : 0;
            }
            ...
            $planData['statusbar']['social_connect']['facebook'] = isset($userSocialConnection) && $userSocialConnection->is_facebook_auth ? 1 : 0;

            $planData['statusbar']['social_connect']['instagram'] = isset($userSocialConnection) && $userSocialConnection->is_instagram_auth ? 1 : 0;

            $planData['statusbar']['social_connect']['twitter'] = isset($userSocialConnection) && $userSocialConnection->is_twitter_auth ? 1 : 0;

            $planData['statusbar']['social_connect']['linkedin'] = isset($userSocialConnection) && $userSocialConnection->is_linkedin_auth ? 1 : 0;

            $planData['statusbar']['social_connect']['youtube'] = (isset($userSocialConnection) && $userSocialConnection->is_youtube_auth) || ($getYoutubeTasks > 0) ? 1 : 0;

            $planData['statusbar']['social_connect']['google'] = isset($userSocialConnection) && $userSocialConnection->is_google_auth ? 1 : 0;
            ...
            $planData['statusbar']['challenge_setting']['instant_challenge'] = (isset($instantChallengeOfferReward) && $instantChallengeOfferReward!=NULL) ? 1 : 0;
            $planData['statusbar']['challenge_setting']['shared_challenge'] = (isset($sharedChallengeOfferReward) && $sharedChallengeOfferReward!=NULL) ? 1 : 0;
            $planData['statusbar']['challenge_setting']['auto_shared_challenge_setting'] = (isset($business_detail->share_to_subscribed_customers) && $business_detail->share_to_subscribed_customers != 0) && (isset($business_detail->auto_share_timing_id) && isset($business_detail->auto_share_timing_id) != NULL) ? 1 : 0;
            ...
            $planData['statusbar']['personalised_msg']['share_new_offer'] = (isset($business_detail->send_when_start) && $business_detail->send_when_start) ? 1 : 0;
            ...
            return view('business.components.settings-statusbar', compact('planData'));
        }
    }
    ...
    static function getBusinessPlanDetails(){
        ...
        $planData['statusbar']['setting']['social_link'] = (isset($planData['business_detail']) && ($planData['business_detail']->facebook_link!=NULL || $planData['business_detail']->instagram_link!=NULL || $planData['business_detail']->twitter_link!=NULL || $planData['business_detail']->linkedin_link!=NULL || $planData['business_detail']->youtube_link)) ? 1 : 0;
        ...
        $planData['statusbar']['challenge_setting']['auto_shared_challenge_setting'] = (isset($planData['business_detail']->share_to_subscribed_customers) && $planData['business_detail']->share_to_subscribed_customers != 0) && (isset($planData['business_detail']->auto_share_timing_id) && isset($planData['business_detail']->auto_share_timing_id) != NULL) ? 1 : 0;
        ...
        $planData['statusbar']['personalised_msg']['share_new_offer'] = (isset($planData['business_detail']->send_when_start) && $planData['business_detail']->send_when_start) ? 1 : 0;
        $planData['statusbar']['personalised_msg']['dob'] = isset($dobMsg->is_scheduled) && $dobMsg->is_scheduled ? 1 : 0;
        $planData['statusbar']['personalised_msg']['anni'] = isset($anniMsg->is_scheduled) && $anniMsg->is_scheduled ? 1 : 0;
    ...
    static function getEmployeePlanDetails($business_id){
        ...
        $planData['BusinessUser'] = User::where('id', $business_id)->first();
