...
class ExpirePlan1EmailJob implements ShouldQueue
{
    ...
    {
        // Log::debug("run check:expireplanoneday => Job file => ExpirePlan1EmailJob => handle datetime: ".date('d-m-Y H:s:i'));

        $one_day_before = Carbon::now()->addDay(1);

        $userIds = UserChannel::whereDate('will_expire_on', '=', $one_day_before)
            ->groupBy('user_id')
            ->pluck('user_id')
            ->toArray();

        $users = User::whereIn('id', $userIds)->where('status', 1)->get();
        // Log::debug("run check:expireplanoneday => Job file => ExpirePlan1EmailJob => handle => check user exist or not datetime: ".date('d-m-Y H:s:i'));

        if (!empty($users)) {
            // Log::debug("run check:expireplanoneday => Job file => ExpirePlan1EmailJob => handle => user exist and go to foreach loop datetime: ".date('d-m-Y H:s:i'));
            ...
            foreach ($users as $key => $user) {
                ...
                $addmin_history->template_name = 'mp_will_1_expire_alert';
                ...
                $payload = \App\Http\Controllers\WACloudApiController::mp_will_1_expire_alert('91'.$user->mobile, $user->name, $day, $shortLinkData->original["code"]);
