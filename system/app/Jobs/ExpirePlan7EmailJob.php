...
class ExpirePlan7EmailJob implements ShouldQueue
{
    ...
    {
        // Log::debug("run check:expireplansevenday => Job file => ExpirePlan7EmailJob => handle datetime: ".date('d-m-Y H:s:i'));
        
        $seven_day_before = Carbon::now()->addDay(7);

        $userIds = UserChannel::whereDate('will_expire_on', '=', $seven_day_before)
            ->groupBy('user_id')
            ->pluck('user_id')
            ->toArray();
        ...
        // Log::debug("run check:expireplansevenday => Job file => ExpirePlan7EmailJob => handle => check user exist or not datetime: ".date('d-m-Y H:s:i'));

        if (!empty($users)) {
            // Log::debug("run check:expireplansevenday => Job file => ExpirePlan7EmailJob => handle => user exist and go to foreach loop datetime: ".date('d-m-Y H:s:i'));
