...
class ExpirePlan5EmailJob implements ShouldQueue
{
    ...
    {
        // Log::debug("run check:checkExpirePlanFiveDay => Job file => ExpirePlan5EmailJob => handle datetime: ".date('d-m-Y H:s:i'));

        $five_day_before = Carbon::now()->addDay(5);

        $userIds = UserChannel::whereDate('will_expire_on', '=', $five_day_before)
            ->groupBy('user_id')
            ->pluck('user_id')
            ->toArray();
        ...
        // Log::debug("run check:checkExpirePlanFiveDay => Job file => ExpirePlan5EmailJob => handle => check user exist or not datetime: ".date('d-m-Y H:s:i'));

        if (!empty($users)) {
            // Log::debug("run check:checkExpirePlanFiveDay => Job file => ExpirePlan5EmailJob => handle => user exist and go to foreach loop datetime: ".date('d-m-Y H:s:i'));
