...
class WalletBalanceZeroJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $user_notification_info = UserNotification::where('notification_id', 3)->where('user_id',$this->user->id)->first();
        
        if($user_notification_info->email==1){
            $email_info = Email::where('id', 11)->first();
            $email_info['content'] = str_replace('Dear [Name]', "<b>". "Dear &nbsp;"  . $this->user->name."</b>", $email_info['content']);
        ...
        if($user_notification_info->wa==1){
            ...
            if($shortLinkData->original["success"] != false){
                ...
                $payload = \App\Http\Controllers\WACloudApiController::mp_no_message_credits('91'.$this->user->mobile, $this->user->name, $shortLinkData->original["code"]);
                $res = \App\Http\Controllers\WACloudApiController::sendMsg($payload);

                
                /* Admin Message History start */
                $addmin_history = new AdminMessage();
                $addmin_history->template_name = 'mp_no_message_credits';
