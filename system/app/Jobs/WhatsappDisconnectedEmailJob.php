...
class WhatsappDisconnectedEmailJob implements ShouldQueue
{
    ...
    {
        // Log::debug("run check:whatsapp => Job file => WhatsappDisconnectedEmailJob => handle datetime: ".date('d-m-Y H:s:i'));
        ...
        $users = User::with('whatsappSession')
                ->has('whatsappSession')
                ->whereIn('id', $userIds)
                ->where('status', 1)
                ->get();
               
        // Log::debug("run check:whatsapp => Job file => WhatsappDisconnectedEmailJob => handle => check users whos whatsapp disconnected datetime: ".date('d-m-Y H:s:i'));

        if(!empty($users)){
            // Log::debug("run check:whatsapp => Job file => WhatsappDisconnectedEmailJob => handle => get users whos whatsapp disconnected datetime: ".date('d-m-Y H:s:i'));
