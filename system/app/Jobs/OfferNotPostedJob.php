...
class OfferNotPostedJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $user_notification_info = UserNotification::where('notification_id', 6)->where('user_id',$this->useroffer->user->id)->first();
        if($user_notification_info->email==1){
            $email_info = Email::where('id', 8)->first();
            $email_info['content'] = str_replace('Dear {customer_name}', "<b>". "Dear &nbsp;"  . $this->useroffer->user->name."</b>", $email_info['content']);
        ...
        if($user_notification_info->wa==1){
            ...
            $shortLinkData = ShortLinkController::callShortLinkApi($long_link, $this->useroffer->user->id ?? 0, "offer_not_posted");

            if($shortLinkData->original["success"] != false){
                /* Send whatsapp notification */
                $payload = \App\Http\Controllers\WACloudApiController::mp_dont_posted_offer('91'.$this->useroffer->user->mobile, $this->useroffer->user->name, $shortLinkData->original["code"]);
