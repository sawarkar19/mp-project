<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title>Business Information Page</title>

    <!-- favicon -->
    <link rel="shortcut icon" href="{{ asset('favicon.ico') }}" type="image/x-icon">
    <!-- Bootstrap 5 -->
    <link rel="stylesheet" href="{{ asset('assets/front/vendor/bootstrap/css/bootstrap.min.css') }}" media="all">
    <!-- ICONS (Bootstrap) V1.5.0 -->
    <link rel="stylesheet" href="{{ asset('assets/front/vendor/bootstrap-icons/font/bootstrap-icons.css') }}" media="all">
    <!-- CUSTOM CSS -->
    <link rel="stylesheet" href="{{ asset('assets/front/css/style.css') }}" media="all">
    <link rel="stylesheet" href="{{ asset('assets/front/css/responsive.css') }}" media="all">
    <link rel="stylesheet" href="{{ asset('assets/front/css/customs.css') }}" media="all">

    <style>

        .binfo_tab{
            border-radius: 20px;
            box-shadow: 0px 5px 20px rgba(0, 0, 0, 0.1);
            margin-bottom: 40px; 
            background: #FFF;
        }
        .footer,
        .binfo_tab{
            position: relative;
            width: 100%;
            max-width: 1024px;
            margin-left: auto;
            margin-right: auto;
            
        }
        .footer-logo{
            width: 100%;
            max-width: 100px;
        }
        .logo img{
            width: 150px;
            max-width: 100%;
        }
        .bg-color-shade{
            position: relative;
            width: 100%;
            padding-top: 50px;
            padding-bottom: 50px;
        }
        .bg-color-shade::before{
            content: "";
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            opacity: 0.3;
            /* z-index: -1; */
            background: var(--color-thm-shd);
        }

        .cover-img{
            position: relative;
            width: 100%;
        }
        .cover-img-thumb{
            position: relative;
            width: 100%;
            padding-bottom: 36%;
            background-color: #F5F5F5;
        }
        .contact-info-list{
            position: relative;
            list-style: none;
            padding-left: 0px;
        }
        .contact-info-list li{
            position: relative;
            padding-left:30px;
            margin-bottom: 10px;
        }
        .contact-info-list li > i{
            position: absolute;
            left: 0;
            top: 0;
            display: block;
            background: var(--color-thm-lth);
            color: #fff;
            width: 25px;
            height: 25px;
            border-radius: 50%;
            text-align: center;
            font-size: 14px;
        }
        .contact-info-list a{
            color: inherit;
            text-decoration: none;
        }
        @media (max-width:767px){
            .bg-color-shade{
                padding-top: 15px;
            }
        }

        .social-links{
            position: relative;
            list-style: none;
            padding-left: 0px;
            text-align: right;
        }
        .social-links li{
            display: inline-block;
            margin: 0 10px 10px 0;
        }
        .social-links li a{
            text-decoration: none;
            color: inherit;
            font-size: 13px;
            padding: .3rem;
            transition: all 300ms ease;
        }
        .social-links li a svg{
            width: 18px;
            height: 18px;
            fill: rgb(37, 37, 37);
            transition: all 300ms ease;
        }
        .social-links li a:hover svg{
            fill: rgb(82, 82, 82);
        }
        .logo-image{
            border:5px solid #e8e8e8;
        }
        @media (max-width:767px) {
            .social-links,
            .copyright{
                text-align: center;
            }
        }
    </style>
</head>
<body>
    
    <div  class="bg-color-shade">
        <div class="container">
            <div class="binfo_tab">
                <div class="py-5">
    
                    <div class="text-center mb-5" id="header">
                        <div class="logo mb-4">
                            <img class="logo-image" src="{{ asset('assets/business/logos/'.$businessDetail->logo) }}" alt="">
                        </div>
                        {{-- <div class="shopname">
                            <h2 class="fw-bold h1 ">{{ $businessDetail->business_name }}</h2>
                            <p class="text-uppercase text-muted ls-2 small">{{ $businessDetail->tag_line }}</p>
                        </div> --}}
                    </div>

                    <div class="qr-code-section">

                        @php

                            // Personal Information
                            $firstName = $businessDetail->business_name;
                            $lastName = '';
                            $title = '';
                            $email = $businessDetail->owner->email;
                            
                            // Addresses
                            $homeAddress = [
                                'type' => 'home',
                                'pref' => true,
                                'street' => '',
                                'city' => '',
                                'state' => '',
                                'country' => '',
                                'zip' => ''
                            ];

                            if($businessDetail->address_line_2 != null){
                                $address = $businessDetail->address_line_1.', '.$businessDetail->address_line_2;
                            }else{
                                $address = $businessDetail->address_line_1 ?? '';
                            }

                            $wordAddress = [
                               'type' => 'work',
                               'pref' => false,
                               'street' => $address ?? '',
                               'city' => $businessDetail->city ?? '',
                               'state' => $businessDetail->state ?? '',
                               'country' => 'India',
                               'zip' => $businessDetail->pincode ?? ''
                            ];
                            
                            $addresses = [$homeAddress, $wordAddress];
                            
                            // Phones
                            $workPhone = [
                                'type' => 'work',
                                'number' => '+91'.$businessDetail->owner->mobile ?? '',
                                'cellPhone' => false
                            ];
                            $homePhone = [
                                'type' => 'home',
                                'number' => '',
                                'cellPhone' => false
                            ];
                            $cellPhone = [
                                'type' => 'work',
                                'number' => '',
                                'cellPhone' => true
                            ];
                            
                            $phones = [$workPhone, $homePhone, $cellPhone];

                        @endphp

                        
                    </div>

                    <style>
                        .qr_div{
                            position: relative;
                            width: 100%;
                            max-width: 280px;
                            margin: auto;
                        }
                        .qr-box{
                            width: 100%;
                            padding: 10px;
                            border: 1px solid var(--bs-success);
                        }
                        .qr-box svg{
                            width: 100%!important;
                            height: auto!important;
                        }
                    </style>
                    <div class="container-fluid mb-5">
                        <div class="row align-items-center">
                            <div class="col-md-6">

                                <div class="qr_div">
                                    <p class="mb-2 text-center" style="line-height: 1.2;"><small><i>Scan this below QR Code to save our contact details.</i></small></p>

                                    <div class="qr-box">
                                        {!! QRCode::vCard($firstName, $lastName, $title, $email, $addresses, $phones)
                                            ->setErrorCorrectionLevel('H')
                                            ->setSize(4)
                                            ->setMargin(2)
                                            ->svg() 
                                        !!}
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="shopname">
                                    @if($businessDetail->business_name!='')
                                        <h2 class="fw-bold h3 ">{{ $businessDetail->business_name }}</h2>
                                    @endif
                                    @if($businessDetail->tag_line!='')
                                        <p class="text-uppercase text-muted ls-2 small">{{ $businessDetail->tag_line }}</p>
                                    @endif
                                </div>
                                <ul class="contact-info-list">
                                    {{-- Owner Name --}}
                                    @if($businessDetail->owner->name!='')
                                       <li><i class="bi bi-person"></i> {{ $businessDetail->owner->name }}</li>
                                    @endif

                                    {{-- EMail ID  --}}
                                    @if($businessDetail->owner->email!='')
                                        <li><i class="bi bi-envelope"></i> <a href="mailto:{{ $businessDetail->owner->email }}">{{ $businessDetail->owner->email }}</a></li>
                                    @endif

                                    {{-- Phone No. --}}
                                    @if($businessDetail->owner->mobile!='')
                                        <li><i class="bi bi-telephone"></i> <a href="tel:{{ $businessDetail->owner->mobile }}">{{ $businessDetail->owner->mobile }}</a> </li>
                                    @endif

                                    {{-- Address  --}}
                                    @if ($businessDetail->address_line_1 != '')
                                        <li><i class="bi bi-geo-alt"></i> <p>{{ $businessDetail->address_line_1 }} {{ $businessDetail->address_line_ }} {{ $businessDetail->city }}, {{ $businessDetail->state }} {{ $businessDetail->country }}  - {{ $businessDetail->pincode }} </p></li>
                                    @endif
                                </ul>
                            </div>
                        </div>
                    </div>
    
                    <div class="cover-img" id="cover_img">
                        <div class="cover-img-thumb" style="background-image: url({{asset('assets/business/Business-profile.png')}});background-size: cover;"></div>
                    </div>
                    

    
                </div>
            </div>
    
            <div class="footer" id="footer">
                <div>
                    <div class="row">
                        <div class="col-md-6 ">
                            <p class="copyright">&copy; {{ date('Y') }} All rights reserved | Powered By 
                                <a href="https://logicinnovates.com/" target="_blank" class="brandname">
                                <span class="color-drk">Logic Innovates</span>
                                </a>
                            </p>
                        </div>
                        <div class="col-md-6">
                            
                            <ul class="social-links mb-2">
                                @if($businessDetail->facebook_link != '')
                                <!-- Facebook  -->
                                <li><a href="{{ $businessDetail->facebook_link }}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"/>
                                    </svg>
                                </a></li>
                                @endif
    
                                @if($businessDetail->instagram_link != '')
                                <!-- Instagram -->
                                <li><a href="{{ $businessDetail->instagram_link }}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z"/>
                                    </svg>
                                </a></li>
                                @endif
                                
                                @if($businessDetail->twitter_link != '')
                                <!-- twitter  -->
                                <li><a href="{{ $businessDetail->twitter_link }}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z"/>
                                    </svg>
                                </a></li>
                                @endif
                                
                                @if($businessDetail->linkedin_link != '')
                                <!-- LinkedIn  -->
                                <li><a href="{{ $businessDetail->linkedin_link }}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/>
                                    </svg>
                                </a></li>
                                @endif
    
                                @if($businessDetail->youtube_link != '')
                                <!-- Youtube  -->
                                <li><a href="{{ $businessDetail->youtube_link }}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.007 2.007 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.007 2.007 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31.4 31.4 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.007 2.007 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A99.788 99.788 0 0 1 7.858 2h.193zM6.4 5.209v4.818l4.157-2.408L6.4 5.209z"/>
                                    </svg>
                                </a></li>
                                @endif
    
                                <!-- Pinterest  -->
                                <!-- <li><a href="#" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M8 0a8 8 0 0 0-2.915 15.452c-.07-.633-.134-1.606.027-2.297.146-.625.938-3.977.938-3.977s-.239-.479-.239-1.187c0-1.113.645-1.943 1.448-1.943.682 0 1.012.512 1.012 1.127 0 .686-.437 1.712-.663 2.663-.188.796.4 1.446 1.185 1.446 1.422 0 2.515-1.5 2.515-3.664 0-1.915-1.377-3.254-3.342-3.254-2.276 0-3.612 1.707-3.612 3.471 0 .688.265 1.425.595 1.826a.24.24 0 0 1 .056.23c-.061.252-.196.796-.222.907-.035.146-.116.177-.268.107-1-.465-1.624-1.926-1.624-3.1 0-2.523 1.834-4.84 5.286-4.84 2.775 0 4.932 1.977 4.932 4.62 0 2.757-1.739 4.976-4.151 4.976-.811 0-1.573-.421-1.834-.919l-.498 1.902c-.181.695-.669 1.566-.995 2.097A8 8 0 1 0 8 0z"/>
                                    </svg>
                                </a></li> -->
    
                            </ul>
    
                        </div>
                    </div>
                </div>
            </div>
    
        </div>
    </div>
    

    <script src="{{ asset('assets/front/vendor/jQuery/jQuery.min.js') }}"></script>
    <script src="{{ asset('assets/front/vendor/bootstrap/js/bootstrap.min.js') }}"></script>
</body>
</html>