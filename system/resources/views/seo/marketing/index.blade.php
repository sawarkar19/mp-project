@extends('layouts.seo')
@section('title', 'SEO: Marketing Tools')
@section('head')
...
<div class="row">
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <form class="basicform" action="{{ route('seo.marketing.store') }}" method="post">
          @csrf
  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <form class="basicform" action="{{ route('seo.marketing.store') }}" method="post">
          @csrf
          <input type="hidden" name="type" value="google-tag-manager">
        <div class="form-group row mb-4">
         <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3" > <a href="https://developers.google.com/tag-platform/tag-manager/web" target="_blank">{{ __('Container ID (GTM-XXXXXX)') }}</a></label>
          <div class="col-sm-12 col-md-7">
            <input type="text" class="form-control" required="" name="gtm_container_id" placeholder="GTM-XXXXXX" value="{{ $info->gtm_container_id ?? '' }}">
          </div>
        </div>
        ...
        <div class="form-group row mb-4">
          <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Status') }}</label>
          <div class="col-sm-12 col-md-7">
            <select class="form-control selectric" name="google_tag_manager_status">
              @if(!empty($info))
              <option value="on" @if($info->google_tag_manager_status  === 'on') selected="" @endif>{{ __('Enable') }}</option>
              <option value="off"  @if($info->google_tag_manager_status  === 'off') selected="" @endif>{{ __('Disable') }}</option>
              @else
              <option value="on">{{ __('Enable') }}</option>
              <option value="off" >{{ __('Disable') }}</option>
              @endif
