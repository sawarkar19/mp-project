          ...
          <div class="table-responsive">
            <table class="table table-striped table-hover text-center table-borderless">
              ...
              <tbody>
                ...
                @else
                  <tr>
                    <td><img src="{{ asset('assets/'.$row->preview->content ?? '') }}" alt="" height="50" width="120"></td>
                    <td>{{ $row->name }}</td>
                    <td>@if($row->status===1) <span class="badge badge-success">{{ __('Active') }}</span> @else <span class="badge badge-danger">{{ __('Deactive') }}</span> @endif</td>
