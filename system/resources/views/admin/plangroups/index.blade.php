...
@section('title', 'Admin: Plan Groups List')
@section('head')
    @include('layouts.partials.headersection', ['title' => 'Plan Groups List'])
@endsection
@section('content')
    <div class="row">
        <div class="col-12 mt-2">
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-sm-8"> <a href="{{ route('admin.plangroups.index') }}"
                                class="mr-2 btn btn-outline-primary @if ($status === ' all ') active @endif">{{ __('All') }}
                                 {{ ($all) }} </a>
                            <a href="{{ route('admin.plangroups.index', 'status=1') }}"
                                class="mr-2 btn btn-outline-success @if ($status == 1) active @endif">{{ __('Active') }}
                                {{ ($actives) }}</a>
                            <a href="{{ route('admin.plangroups.index', 'status=0') }}"
                                class="mr-2 btn btn-outline-warning @if ($status == 0) active @endif">{{ __('Inactive') }}
                                {{ ($inactives) }}</a>
                        </div>
                        <div class="col-sm-4 text-right">
                            <a href="{{ route('admin.plangroups.create') }}"
                                class="btn btn-primary">{{ __('Create Plan Group') }}</a>
                        </div>
                    </div>
                    ...
                    <form method="post" action="{{ route('admin.plangroups.destroys') }}" id="actionForm">
                        @csrf
                                            ...
                                            <tr id="row{{ $group->id }}">
                                                ...
                                                <td>
                                                    <button class="btn btn-primary has-icon"
                                                        onclick="window.location.href = '{{ route('admin.plangroups.edit', $group->id) }}'"
                                                        type="button"> <i class="fas fa-edit"></i> {{ __('Edit') }}
                                                    </button>
                                                    <button class="btn btn-success has-icon"
                                                        onclick="window.location.href = '{{ route('admin.plangroups.show', $group->id) }}'"
                    ...
                    {{ $plangroups->appends($request->all())->links('vendor.pagination.bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
