...
<div class="row">
    <div class="col-12 mt-2">
        <div class="card">
            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-sm-8"> <a href="{{ route('admin.states.index') }}" class="mr-2 btn btn-outline-primary @if($status===" all ") active @endif">{{ __('All') }} ({{ $all }})</a>
                        <a href="{{ route('admin.states.index','status=1') }}" class="mr-2 btn btn-outline-success @if($status==1) active @endif">{{ __('Active') }} ({{ $actives }})</a>
                        <a href="{{ route('admin.states.index','status=0') }}" class="mr-2 btn btn-outline-warning @if($status==0) active @endif">{{ __('Inactive') }} ({{ $inactives }})</a>
                    </div>
                    <div class="col-sm-4 text-right">
                        <a href="{{ route('admin.states.create') }}" class="btn btn-primary">{{ __('Create State') }}</a>
                    </div>
                </div>
                ...
                <form method="post" action="{{ route('admin.states.destroys') }}" class="basicform_with_reload">
                    @csrf
                            ...
                            <tbody>
                                @foreach($states as $row)
                                    <tr id="row{{ $row->id }}">
                                        ...
                                        <td>
                                            <button class="btn btn-primary has-icon" onclick="window.location.href = '{{ route('admin.states.edit',$row->id) }}'" type="button"> <i class="fas fa-edit"></i> {{ __('Edit') }}</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                {{ $states->appends($request->all())->links('vendor.pagination.bootstrap-4') }}</div>
        </div>
    </div>
</div>
@endsection
