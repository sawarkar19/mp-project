...
<script type="text/javascript">
    ...
    function setNormalTab(instant_task_id, task_key){
        ...
        setNormalTabCookie(instant_task_id, task_key);
    }

    function setNormalTabCookie(instant_task_id, task_key){
        // Update Cookies for normal Tab.
        var urlParamsObject = new URLSearchParams(window.location.search)
        var urlParamsString = urlParamsObject.toString();
        var share_cnf = urlParamsObject.get('share_cnf');

        let cName = "oc-"+share_cnf+"-tasks";
        
        let oldTaskCard = getCookie(cName);
        if(oldTaskCard!=undefined || oldTaskCard!=null){
            oldTaskCard = $.parseJSON(oldTaskCard);
            for (var i = 0; i < oldTaskCard.length; i++){
                if(oldTaskCard[i].task_key == task_key){
                    // Update Old
                    oldTaskCard[i].task_key = task_key;
                    oldTaskCard[i].instant_task_id = instant_task_id;
                    oldTaskCard[i].status = "normal";

                    newItem = false;
                    setTasksCookie(cName, JSON.stringify(oldTaskCard), 5);
                    // saveCookiesInLive("update", task_key, instant_task_id, "normal");
                }
            }
        }
    }
    ...
    function setUnverifiedTask(instant_task_id, task_key){
        ...
        // store cookies for verifying
        setTaskStatusInCookies(task_key, instant_task_id, "verifying_failed");
    }
    ...
    function setVerifiedTask(instant_task_id, task_key){
        ...
        // store cookies for verifying
        setTaskStatusInCookies(task_key, instant_task_id, "verified");
    }

    // Show Task Complete Anumation and sent Redeem code
    function autoSendRedeemCode(requestFrom="showMsg"){
        $("#forResend").show();
        $("#redeemMsg").html('');
        /*getting all url with parameters*/
        var urlParamsObject = new URLSearchParams(window.location.search)
        ...
        $.ajax({
            ...
            success: function (results) {
                ...
                if (results.status === true && results.type == 'success') {
                    congrats();
                    localStorage.setItem("oc-"+share_cnf+"-redeem", results.status);

                    $("#redeemMsg").html('<span class="text-success">'+results.message+'</span>');
                }
                else{
                    if(requestFrom=="showMsg"){
                        $("#redeemMsg").html('<span class="text-danger">'+results.message+'</span>');
                    }
                }
            },
            error: function(xhr) { 
    ...
    function checkTask(task_key, instant_task_id){
        ...
        // store cookies for verifying
        setTaskStatusInCookies(task_key, instant_task_id, "verifying");
    ...
    function checkGoogleReview(instance_task_id) {
        ...
        var tasklink = $("#instant_task_id_"+instance_task_id).attr("data-link");
        ...
        setLaoding();
        getContribeIdUrl(task_key, tasklink);
    }

    function getContribeIdUrl(task_key, tasklink){
        var urlParamsObject = new URLSearchParams(window.location.search)
        var urlParamsString = urlParamsObject.toString();
        var share_cnf = urlParamsObject.get('share_cnf');
            ...
            success: function (results) {
                if(results.status==true){
                    ...
                    if (newWindow == null || typeof(newWindow)=='undefined') {
                        ...
                        localStorage.setItem("oc-review-link", tasklink); 
...
<script>
    ...
    // Instant Task Status Cookies
    // store cookies for verifying
    function setTaskStatusInCookies(task_key, instant_task_id, status="verifying"){
        var urlParamsObject = new URLSearchParams(window.location.search)
        var urlParamsString = urlParamsObject.toString();
        var share_cnf = urlParamsObject.get('share_cnf');
        ...
        if(oldTaskCard!=undefined || oldTaskCard!=null){
            ...
            for (var i = 0; i < oldTaskCard.length; i++){
                if(oldTaskCard[i].task_key == task_key){
                    ...
                    oldTaskCard[i].task_key = task_key;
                    oldTaskCard[i].instant_task_id = instant_task_id;
                    oldTaskCard[i].status = status;

                    newItem = false;
                    setTasksCookie(cName, JSON.stringify(oldTaskCard), 5);
                    saveCookiesInLive("update", task_key, instant_task_id, status);
                }
                // else{
                //     // New Entry
                //     oldTaskCard.push({'task_key': task_key, 'instant_task_id': instant_task_id, status: status});
                //     console.log("add entry");
                // }
                // console.log(oldTaskCard[i]);
            }
            ...
            if (newItem) {
                ...
                setTasksCookie(cName, JSON.stringify(oldTaskCard), 5);
                newItem = false;
                saveCookiesInLive("new", task_key, instant_task_id, status);
            }
        }

        // New Entry
        if (newItem) {
            taskCards.push({'task_key': task_key, 'instant_task_id': instant_task_id, status: status});
            setTasksCookie(cName, JSON.stringify(taskCards), 5);
    ...
    // Set a Cookie
    function setTasksCookie(cName, cValue, expDays) {
        let date = new Date();
    ...
    function saveCookiesInLive(status="new", task_key, instant_task_id, task_status="verifying"){
        var urlParamsObject = new URLSearchParams(window.location.search)
        var urlParamsString = urlParamsObject.toString();
        var share_cnf = urlParamsObject.get('share_cnf');
        ...
        // Cookie name
        let cName = "oc-"+share_cnf+"-tasks";
        let taskStatus = getCookie(cName);

        if(taskStatus!=undefined || taskStatus!=null){
            $.ajax({
                type: 'POST',
                url: "{{ route('setTaskStatistics') }}",
                data: {
    ...
    function getAndSetDbCookie(){
        ...
        $.ajax({
            ...
            success: function (results) {
                ...
                console.log(results);
                let cName = "oc-"+share_cnf+"-tasks";
                setTasksCookie(cName, JSON.stringify(results.data), 5);
                setTaskStatusUsingCookies(share_cnf);
            },
            error: function(xhr) { 
                console.log("set cookies error =>");
                console.log(xhr);
    ...
    function setTaskStatusUsingCookies(share_cnf){
        let cName = "oc-"+share_cnf+"-tasks";
        let taskStatus = getCookie(cName);

        if(taskStatus!=undefined || taskStatus!=null){
            taskStatus = $.parseJSON(taskStatus);
            for (var i = 0; i < taskStatus.length; i++){

                var classList = $("#instant_task_id_"+taskStatus[i].instant_task_id).attr("class");
                $("#instant_task_id_"+taskStatus[i].instant_task_id).attr('data-classes', classList);
                $("#instant_task_id_"+taskStatus[i].instant_task_id).attr('class', '');
                
                // Google Review
                $("#instant_task_id_"+taskStatus[i].instant_task_id).removeAttr("href");
                $("#instant_task_id_"+taskStatus[i].instant_task_id).removeAttr("onclick");
                $("#instant_task_id_"+taskStatus[i].instant_task_id).removeAttr("onClick");
                
                // reverify
                $("#set-single-task_"+taskStatus[i].instant_task_id).removeClass("unverify");

                $("#set-single-task_"+taskStatus[i].instant_task_id).addClass("verifying");
                $("#instant_task_id_"+taskStatus[i].instant_task_id).addClass("disabled");
                
                if(taskStatus[i].status == "verifying_failed" || taskStatus[i].status == "verifying"){
                    setUnverifiedTask(taskStatus[i].instant_task_id, taskStatus[i].task_key);
                }
                else if(taskStatus[i].status == "verified"){
                    setVerifiedTask(taskStatus[i].instant_task_id, taskStatus[i].task_key);
                }
                else{
                    setNormalTab(taskStatus[i].instant_task_id, taskStatus[i].task_key);
                }
            }
        }
    }
    ...
    $(document).ready(function(){
        ...
        // Redeem Code Cookie name
        var isRedeemed = localStorage.getItem("oc-"+share_cnf+"-redeem");
        if(isRedeemed=='true'){
            $("#forResend").show();
        }
