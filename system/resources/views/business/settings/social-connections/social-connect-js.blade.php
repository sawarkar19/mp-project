...
<script>
    ...
    function connectGoogleReview(){
        ...
        htmlTag += '<hr />';
        htmlTag += '<div class="text-left">Place Id <a href="#" class="info-btn" data-toggle="modal" data-target="#google_details_modal1" data-backdrop="static" data-keyboard="false"><i class="fa fa-info-circle"></i></a></div>';
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response.status==200){
                        currentOfferPostToSocialMedia("facebook");
        ...
        $(document).on("submit", "#linkedinPagesForm", function(e){
            ...
            $.ajax({
                ...
                success: function (response) {
                    // console.log(response);
                    if(response.status==200){
                        currentOfferPostToSocialMedia("linkedIn");
    ...
    function getTwitterUsername(){
        ...
        $.ajax({
            ...
            success: function (response) {
                // console.log(response);
                $("#overlay").fadeOut(300);
                currentOfferPostToSocialMedia("twitter");
            }
        });
    }
    ...
    function getInstagramUsername() {
        ...
        $.ajax({
            ...
            success: function (response) {
                $("#overlay").fadeOut(300);
                currentOfferPostToSocialMedia("instagram");
            }
        });
    }

    $(document).on("click", "#showAccountProfile", function(){
        var username =$(this).attr("data-username");
        var modaltitle =$(this).attr("data-modaltitle");
        getDetails(modaltitle, username);
    });
    ...
    function getDetails(modaltitle, username){
        $("#mediaModalTitle").text(modaltitle);
        $("#mediaModalValue").text(username);
        $("#showMediaModal").modal('show');
    }

    function currentOfferPostToSocialMedia(socialType=""){
        $('.refresh-settings-status').click();
        sessionStorage.setItem("setting_section", "social_connection");
        $.ajax({
            url: '{{ route("business.onConnectPostToSocialMedia") }}',
            type: "POST",
            data: {
                _token: '{{ csrf_token() }}',
