            ...
            <div class="row justify-content-center_">
                ...
                <div class="col-md-6">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="post" class="ProfileForm basicSettingform"
                                action="{{ route('business.profileUpdate') }}">
                                @csrf
                                <div class="custom-form">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name" id="name" class="form-control" required
                                            placeholder="Enter User's  Name" value="{{ Auth::user()->name }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="email" class="col-form-label">{{ __('Email ID') }}
                                            <span>*</span></label>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="post" id="ChangePasswordForm" class="mb-0 basicSettingform"
                                action="{{ route('business.profileUpdate') }}">
                                {{-- @csrf --}}
                                <div class="custom-form">
                                    ...
                                    <div class="form-group show_hide_password">
                                        ...
                                        <div class="input-group" id="show_hide_old_password">
                                            ...
                                            <div class="input-group-append">
                                                <a href="" class="btn btn-primary"><i class="fa fa-eye-slash"
                                    ...
                                    <div style="padding-bottom: 15px;font-size:10px">
                                        <span><b>Note: </b></span><span style="color: red">If you update your password, you
                                            will be logged out and redirected to Sign In page where you have to login with
                                            New Password.</span>
                                    </div>
                                    {{-- <input type="hidden" id="progress_percent" name="progress_percent" value="{{ //$progress }}" > --}}
                                    <div class="form-group mb-0">
                                        <button type="submit" class="btn btn-info px-4 basicbtn"
                                            name="Change">{{ __('Change') }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div class="col-md-6">
                    <!-- Mobile and email edit -->
                    @if (Auth::user()->role_id != 1)
                        <div class="card">
                            <div class="card-header justify-content-between">
                                <div>
                                    <i class="fa fa-mobile-alt mr-3"></i>
                                    <h4 class="d-inline-block">{{ __('Registered (WhatsApp) Number') }}</h4>
                                </div>

                                <span class="info-btn" data-toggle="tooltip"
                                    title="If you want to update your number please update it from here."><i
                                ...
                                <form method="post" id="basicformMobile">
                                    ...
                                    <div class="custom-form">
                                        <div class="form-group" id="enter_input">
                                            <label for="name" id="mobile_label"
                                                class="col-form-label">{{ __('Number ') }} <small>(10 Digit)</small>
                                                <span>*</span></label>
                                            <label for="name" id="resend_name" style="display:none;"
                                                class="col-form-label">{{ __('Resend OTP Number') }}
                                                <span>*</span></label>
                                            <input type="text" name="mobile" id="mobile"
                                                class="form-control number-validation not-allow-spaces" required
                                                placeholder="Enter WhatsApp Number" value="{{ Auth::user()->mobile }}"
                                                maxlength="10">
                                        </div>
                                        <div class="form-group" id="verify_input" style="display:none;">
                                            <label for="name" class="col-form-label">{{ __('Enter OTP Number') }}
                                                <span>*</span></label>
                                            <input type="text" name="otp" id="otp"
                                                class="form-control number-validation not-allow-spaces" required
                                                placeholder="Enter OTP Number" value="" maxlength="6">
                                        </div>
                                        <div class="form-group" id="OtpBtn">
                                            <button type="button" class="btn btn-info basicbtn1"
                                                onClick="sendOTP();">{{ __('Send OTP') }}</button>
                                        </div>
                                        <div class="form-group" id="VrfBtn" style="display:none;">
                                            <button type="button" class="btn btn-info basicbtn2"
                                                onClick="verifyOTP();">{{ __('Verify OTP') }}</button>
                                            <button type="button" class="btn btn-info basicbtn2"
                                                onClick="resendOTP();">{{ __('Resend OTP') }}</button>
                                        </div>
                        ...
                        <div class="card">
                            <div class="card-header justify-content-between">
                                <div>
                                    ...
                                    <h4 class="d-inline-block">{{ __('Registered Email ID') }}</h4>

                                </div>

                                <span class="info-btn" data-toggle="tooltip"
                                    title="If you want to update your email please update it from here."><i
                                        class="fa fa-info-circle"></i>
                                </span>

                            </div>
                            <div class="card-body">
                                <form method="post" id="basicformEmail">
                                    @csrf
                                    <div class="custom-form">
                                        <div class="form-group" id="enter_input_email">
                                            <label for="name" id="email_label"
                                                class="col-form-label">{{ __('Email ID ') }}
                                                <span>*</span></label>
                                            <label for="name" id="resend_name_email" style="display:none;"
                                                class="col-form-label">{{ __('Resend Email ID') }} <span>*</span></label>
                                            <input type="text" name="email" id="emails"
                                                class="form-control check-email-input not-allow-spaces" required
                                                placeholder="Enter email id" value="{{ Auth::user()->email }}"
                                                maxlength="120">
                                        </div>
                                        <div class="form-group" id="verify_input_email" style="display:none;">
                                            <label for="name"
                                                class="col-form-label">{{ __('Enter Verification Code') }}
                                                <span>*</span></label>
                                            <input type="text" name="otp" id="otp_email"
                                                class="form-control number-validation not-allow-spaces" required
                                                placeholder="Enter Verification Code" value="" maxlength="6">
                                        </div>
                                        <div class="form-group" id="OtpEmailBtn">
                                            <button type="button" class="btn btn-info basicbtn1"
                                                onClick="sendCode();">{{ __('Send Email Verification Code') }}</button>
                                        </div>
                                        <div class="form-group" id="VrfEmailBtn" style="display:none;">
                                            <button type="button" class="btn btn-info basicbtn2"
                                                onClick="verifyCode();">{{ __('Verify Code') }}</button>
                                            <button type="button" class="btn btn-info basicbtn2"
                                                onClick="resendCode();">{{ __('Resend Code') }}</button>
                                        </div>
                                        <div class="form-group"> <span class="successEmail"></span>
