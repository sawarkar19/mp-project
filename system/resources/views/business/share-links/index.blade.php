...
@section('title', 'Send Challenges: Business Panel')
@section('head')
    ...
    <div class="section-body">
        <div class="row">
            ...
            <div class="col-md-6 mb-4">
                <div class="card mb-0">
                    <div class="card-header justify-content-between">
                        <h4>Send Instant Challenge</h4>
                        <span class="info-btn" data-toggle="tooltip"  title="Enter these basic customer’s details to share your current Instant Challenge Offer."><i class="fa fa-info-circle"></i></span>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-12 col-md-6 mb-4">
                <div class="card information mb-0_ @if($planData['userData']->current_account_status == 'free') __pro__ @endif">
                    <div class="card-header justify-content-between ">
                        <h4>Redeem Code</h4>
                        <span class="info-btn" data-toggle="tooltip"  title="All types of offers and challenges can be redeemed here.Just ask your customer to share the redeem code with you and enter it."><i class="fa fa-info-circle"></i></span>
                    </div>
                    <div class="card-body">
                ...
                <div class="card information mb-0_ @if($planData['userData']->current_account_status == 'free') __pro__ @endif">
                    <div class="card-header justify-content-between">
                        <h4>Resend Redeem Code</h4>
                        <span class="info-btn" data-toggle="tooltip"  title="Resend the redeem code manually to a customer who completed the challenge successfully but still have not received the redeem code."><i class="fa fa-info-circle"></i></span>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="" id="resendredeemCodeForm">
                            <div class="form-group">
                                <label>Enter Whatsapp Number <span style="color: red;">*</span></label>
                                <div class="input-group">
                                    <input type="text" name="resendmobile" id="resendmobile" class="form-control indian-mobile-series" maxlength="10" placeholder="Enter whatsapp number" >
                                </div>
                                <span class="text-danger error-section" id="searchResendMobileError"></span>
                            </div>

                            @if($userBalance['wallet_balance'] > 0 )
                                <button type="submit" class="btn btn-success" id="searchResendCode"><i class="fa fa-search"></i> &nbsp;Search</button>
                            @else
                                <button type="button" class="btn btn-success" id="zeroBalence1"><i class="fa fa-search"></i> &nbsp;Search</button>
                            @endif
                        </form>

                        <form method="POST" action="" id="resendRedeemLinkForm">
                            <div class="table table-responsive" id="resendRedeemLinkDiv" style="display: none">
...
// console.log(planData);
       ...
       //get Customer info
        ...
        $('#redeemCodeCheckForm').on('submit', function(event) {
           ...
           var btn = $('#codeApply');
            ...
            $.ajax({
                url: "{{ route('business.applyRedeemCode') }}",
                type: "post",
                dataType: "json",
                data: $data,
                beforeSend: function() {
        ...
        $('#calculate').on('click', function(event) {
            ...
            // console.log(invoice_required.value);
        ...
        $('#redeemForm').on('submit', function(event) {
            ...
            // var billTotal = $("#billTotal").text(), 10);
            ...
            $.ajax({
                url: "{{ route('business.redeemOffer') }}",
                type: "post",
                dataType: "json",
                data: $data,
                beforeSend: function() {
        ...
        // Resend Redeem code
        $(document).on("click", "#searchResendCode", function(e){
            e.preventDefault();
            $("#resendRedeemLinkDiv").hide();
            $("#overlay").fadeIn(300);
            var mobile = $("#resendmobile").val();
            var filter = /^((\+[1-9]{1,4}[ \-]*)|(\([0-9]{2,3}\)[ \-]*)|([0-9]{2,4})[ \-]*)*?[0-9]{3,4}?[ \-]*[0-9]{3,4}?$/;

            if(mobile.length != 10){
                $('#searchResendMobileError').text("Please enter valid number");
                // return false;
            }
            $("#resendData").html("");
            ...
            $.ajax({
                url: "{{ route('business.getRedeemCodeByMobile') }}",
                type: "post",
                dataType: "json",
                data: $data,
                success: function(response) {
                    ...
                    if(response.status==false){
                        ...
                        if(response.data.length!=0){
                            response.data.forEach(element => {
                                htmlResend += "<tr>";
                                    htmlResend += "<td>" + element.created_at + "</td>";

                                    if(element.channel_id == 2){
                                        htmlResend += "<td>Instant Challenge</td>";
                                    }
                                    else{
                                        htmlResend += "<td>Share Challenge</td>";
                                    }

                                    htmlResend += "<td>";
                                        htmlResend += "<a href='javascript:void(0)' class='btn btn-primary resendRedeemBtn' data-id='"+element.id+"' data-channel_id='"+element.channel_id+"' data-offer_subscription_id='"+element.offer_subscription_id+"' ><i class='fa fa-paper-plane'></i> Resend</a>"; 
                                    htmlResend += "</td>";
                                htmlResend += "</tr>";
                            });
                        }
                            ...
                            htmlResend += "<tr>";
                                htmlResend += '<td colspan="3"> {{ Config::get('constants.no_record_found') }}</td>';
                            htmlResend += "</tr>";
                        }
                        $("#resendRedeemLinkDiv").show();
                        $("#resendData").html(htmlResend);
                    }
                },
                error: function(response) {
                    //
        ...
        $(document).on("click", ".resendRedeemBtn", function(e){

            var redeem_id = $(this).attr('data-id');
            var offer_subscription_id = $(this).attr('data-offer_subscription_id');
            var channel_id = $(this).attr('data-channel_id');
            var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
            var isChannelActive = {{ $isChannelActive['status'] }}

            if(channel_id == 2 && isChannelActive == 0){
                var msg = "{{Config::get('constants.instant_challenge_status')}}"
                Sweet("error", msg);
                return false;
            }
            ...
            var $data = {
                ...
                'offer_subscription_id': offer_subscription_id,
                '_token' : CSRF_TOKEN
            };
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                url: "{{ route('business.resendRedeemCode') }}",
                type: "post",
                dataType: "json",
                data: $data,
                success: function(response) {
    ...
    function checkRewardSetting(){

        Swal.fire({
            title: '<strong>Instant Challenge Settings</strong>',
            icon: 'info',
            html:
                'Go to Instant Challenge setting page to update the Challenge settings.',
            ...
            if (result.value == true) {
                window.location.href = '{{ url('/business/channel/2/instant-rewards' ) }}'
            } 
        });
    }; 
    ...
    function sendForPayment(){
        ...
        Swal.fire({
            ...
            icon: 'info',
            html:
                'You do not have Instant challenge access. Please upgrade your Plan to use Share Challenge.',
            showCloseButton: false,
            showCancelButton: true,
            focusConfirm: false,
            allowOutsideClick:false,
...
<script>
    $(document).on('click', '#zeroBalence, #zeroBalence1', function(){
        ...
        var is_paid = '{{ auth()->user()->is_paid }}';
        var current_account_status = '{{ auth()->user()->current_account_status }}';
        if(is_paid == '1' && $(this).attr('id') == 'zeroBalence' && current_account_status == 'paid'){
            Swal.fire({
