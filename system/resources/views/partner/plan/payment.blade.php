...
<div class="mb-5">
	...
	<div class="row justify-content-center">
		<div class="col-xl-8 col-md-10">
			...
			@php
			if(url('/') == env('APP_URL')){
				$url=url('/business/partner/make-charge/');
			}
			else{
				$url=url('/business/partner/make-charge/');
			}
				...
				@if($row->slug == 'stripe')
					...
					<form action="{{ url($url.'/'.$planDetails['plan_id']) }}" method="post" id="payment-form">
						...
						<div class="form-group">
							...
							@if($planData['payble_price'] > 0)
								<button type="submit" class="subscribe btn btn-primary btn-block shadow-sm mt-2"> Make Payment With {{ $row->name }} <span class="h6 font-weight-bold text-success" id="PayablePrice"></span> </button>
							@else
				...
				@else
					...
					<form role="form" method="post" action="{{ url($url.'/'.$planDetails['plan_id']) }}" id="paymentForm">
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<input type="text" name="name" value="{{ $userDetail->name }}" class="form-control" style="height: 40px;" id="name" readonly="">
							</div>
						</div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<input type="text" name="email" value="{{ $userDetail->email }}" class="form-control" style="height: 40px;" id="email" readonly="">
							</div>
						</div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<input type="number" name="phone" value="{{ $userDetail->mobile }}" class="form-control" style="height: 40px;" id="phone" readonly="">
							</div>
						</div>
						...
						<div class="text-right_">
							@if($planData['payble_price'] > 0)
								<div class="d-sm-flex justify-content-between align-items-center">
									<p class="mb-1 mb-sm-0">{{ __('Make Payment With') }} {{ $row->name }}<br><span class="h6 font-weight-bold text-success">{{ $currency_name }}</span> <span class="h6 font-weight-bold text-success" id="PayablePrice"></span> (Inclusive GST).</p>
					...
					<a type="a" class="btn btn-success text-white" href="{{ route('business.partner.paymentLink', $paymentLink->id) }}">Send Link On Whatsapp</a>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			if (sessionStorage.getItem('payable_amount')) {
				...
				window.location.href = '{{ route('business.partner.index') }}';
			}
