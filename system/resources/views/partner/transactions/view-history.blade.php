...
@section('title', 'Transaction History: Partner Panel')
        ...
        <div class="invoice">
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="invoice-title">
                  ...
                  <div class="invoice-number invoice_ID">Invoice ID: {{ $post->invoice_no }}</div>
                </div>
                <hr>
                <div class="row">
                  <div class="col-md-6">
                  ...
                  <div class="col-md-6 text-md-right">
                    <address>
                      ...
                      {{ \Carbon\Carbon::parse($post->created_at)->format('j F, Y') }}<br><br>
                    </address>
                  </div>
                </div>
              </div>
              ...
              <div class="col-md-12">
                ...
                <div class="table-responsive">
                  <table class="table table-striped table-hover table-md">
                    ...
                    @php
                        $sr=1;
                    @endphp
                    @foreach($post->userplans_channel as $plan)
                      <tr>
                        <td>{{ $sr }}</td>
                        <td>
                          {{ @$plan->channel->name }}
                        ...
                        <td class="text-center">
                          @if($post->employee_count > 0 && $plan->feature_id == 6)
                              {{ $post->employee_count }}
                          @elseif($post->direct_post_count > 0 && $plan->feature_id == 7)
                              {{ $post->direct_post_count }}
                          @else
                              -
                          @endif
                        </td>
                ...
                <div class="row mt-4">
                  <div class="col-xl-8 col-md-5 order-2 order-md-1">
                    ...
                    <p class="section-lead">{{ $post->method->name }}</p>
                  </div>
                  ...
                  <div class="col-xl-4 col-md-7 order-1 order-md-2">
                    ...
                    <div class="invoice-detail-item">
                      ...
                      <div class="invoice-detail-value text-right"><span>₹</span>{{ $post->total_amount }}</div>
                    </div>
                    @if($post->added_discount_amount >= 1)
                    <div class="invoice-detail-item">
                      <div class="invoice-detail-name">All Apps 25% off </div>
                      <div class="invoice-detail-value text-right"> - <span>₹</span>{{ $post->added_discount_amount }}</div>
                    </div>
                    @endif

                    @if($post->promocode_amount >= 1)
                    <div class="invoice-detail-item">
                      <div class="invoice-detail-name">Promo Code Discount  </div>
                      <div class="invoice-detail-value text-right"> - <span>₹</span>{{ $post->promocode_amount }}</div>
                    </div>
                    @endif
                    <hr class="mt-2 mb-2">
                    @php
                      ...
                      if ($post->transaction_amount >= 1){
                        $gst_18 = $post->transaction_amount - ($post->transaction_amount * (100/(100 + 18)));
                        $gst = round($gst_18, 2);
                        $sgst= round($gst/2, 2);
                        $cgst= round($gst/2, 2);

                        $tax_amo = round($post->transaction_amount - $gst, 2);
                      }
                    @endphp
                    <div class="invoice-detail-item">
                    ...
                    <div class="invoice-detail-item">
                      ...
                      <div class="invoice-detail-value invoice-detail-value-lg text-right"><span>₹</span>{{ $post->transaction_amount }}</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="row">
            <div class="col-6 text-md-left invoice_button">
              <a href="{{ route('business.partner.viewInvoice',$post->id) }}" class="btn btn-primary btn-icon icon-left"><i class="fas fa-eye"></i> View Invoice</a>
            </div>
            <div class="col-6 text-md-right invoice_button">
              <a href="{{ route('business.partner.downloadHistory',$post->id) }}" class="btn btn-primary btn-icon icon-left"><i class="fas fa-download"></i> Download Invoice</a>
            </div>
          </div>
